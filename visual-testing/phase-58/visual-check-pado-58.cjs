/* Rule 12 visual check, phase 58: the approvals tab must render the sensitive
...
(async () => {
  ...
  const outDir = '/home/user/IGBZ-WP/visual-testing/phase-58';
  ...
  await page.screenshot({ path: path.join(outDir, 'pado-approvals-pending.png'), fullPage: true });
  ...
  const pendingChecks = await page.evaluate(() => {
    ...
    return {
      ...
      seesPriceChange: rows.some((t) => t.includes('price_change') || t.includes('تغییر قیمت')),
      seesBulkDelete: rows.some((t) => t.includes('bulk_product_delete') || t.includes('حذف انبوه')),
      seesKinds: rows.filter((t) => t.includes('تغییر قیمت') || t.includes('حذف انبوه') || t.includes('بازپرداخت')).length,
  ...
  if (histLink) {
    ...
    await page.screenshot({ path: path.join(outDir, 'pado-approvals-history.png'), fullPage: true });
