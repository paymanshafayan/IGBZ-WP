...
| شناسه | نیاز | سند/منبع | کد/آزمون فعلی | وضعیت و شکاف |
|---|---|---|---|---|
| QUE-001 | صف بادوام با claim و retry | WP-Cron source؛ الگوی لیز/پشت‌بند | جدول `jobs` + `JobQueue` (فاز ۲۳) | ✅ هسته: ادعای لیزدار با بازگشت پس از انقضا، تلاش مجدد با پشت‌بند نمایی و لرزش، بن‌بست برای سمی‌ها؛ فاز ۲۴ کارهای پنج‌دقیقه‌ای را منتقل کرد؛ ساعتی و روزانه در فازهای ۲۵ و ۲۶ |
