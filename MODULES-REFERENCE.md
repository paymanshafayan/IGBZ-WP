...
**آخرین به‌روزرسانی:** ۱۴۰۵/۰۶/۰۶ (2026-08-28) · **نسخهٔ افزونه:** 1.0.0 · **دیتابیس:** v19 — ۷۲ جدول
· **ویرا:** ۰.۹.۷ (`vira/`، بخش ۷.۵ در `PROJECT-STATE.md`)

> **معماری هدف:** DeepInfra مستقل هر فروشگاه برای inference و Zernio مرکزی با profile جدا
> برای هر فروشگاه، به‌عنوان تنها provider اجتماعی. اجزای Manus، ChatPlace و ManyChat که پایین‌تر در موجودی کد آمده‌اند بدهی migration هستند،
> نه providerهای هدف؛ پیشنهاد session اینستاگرام در Agent Reach نیز رد شده است.
> مرجع: `ADR/ADR-0004-PADO-ZERNIO-SOCIAL-ARCHITECTURE.md`.
...
کد منبع رفتار فعلی و ADR پذیرفته‌شده منبع معماری هدف است؛ اختلاف آن‌ها شکاف migration است
و نباید با ادعای تکمیل پنهان شود.
...
**وضعیت فعلی کد چندمستأجری:** تک‌سایت با ستون `tenant_id` (نه WordPress Multisite).
تقریباً همهٔ جدول‌ها `tenant_id` دارند؛ فهرست استثناهای عمدی در `tests/SchemaTest.php` ثبت
شده. محصولات و سفارش‌های ووکامرس با متای `_igbz_tenant_id` در کاتالوگ، کوئری سفارش،
فهرست‌های پیشخوان و مسیرهای REST محدوده‌بندی می‌شوند (۰/نبود = مشترک پلتفرم و برای مالک
مستأجر قابل‌نمایش نیست).

**معماری مقصد پذیرفته‌شده در فاز ۰۲:** وردپرس چندسایتی با یک زیرسایت برای هر فروشگاه و
control plane مشترک؛ مرجع `ADR/ADR-0001-MULTISITE-TENANCY.md`. متن بالا فقط موجودی کد پیش
از migration است، نه طراحی نهایی. طبقه‌بندی ۷۲ جدول، تبدیل resolver/provision و انتقال
محصول/سفارش/قالب به مرز زیرسایت هنوز کد ندارد و در کارهای باقی‌مانده می‌ماند.

**مرز مقصد پادو:** طبق ADR-0004 پادو Playbookمحور است و هر زیرسایت حساب/هزینهٔ مستقل
DeepInfra دارد؛ credential آن در IGBZ ذخیره نمی‌شود. ماژول `pado` افزونه فقط قرارداد
connector، API ابزار محدود، اعتبارسنج، preview و مرکز مجوز است؛ runtime inference در IGBZ
یا ویرا نیست. n8n فقط گردش‌کار قطعی مشترک است. خروجی طراحی طبق ADR-0003 سه‌گانه است؛
هیچ‌کدام هنوز چرخهٔ کامل پذیرش خود را ندارند و adapter مقصد نیز در کد پیاده نشده است.
...
| `instagram` | اتوماسیون اینستاگرام | هدف: Zernio برای انتشار/Inbox/analytics و پادو برای تحلیل/تولید؛ کد فعلیِ Manus/ManyChat/ChatPlace نیازمند migration |
...
| `fx` | واسط پرداخت ارزی | زیرساخت FX موجود؛ متراژ و قبض providerهای اجتماعی قدیمی legacy و نیازمند migration است |
| `pado` | مرکز پادو | صف مجوز و قالب فعلی؛ موتور Playbook و adapter مستقل DeepInfra هنوز پیاده نشده است |

ترتیب بایندینگ ماژول‌ها در `Plugin::module_map()`: `multitenant → instagram → hub → fx → rest_api → pado`
...
**معماری هدف:** Zernio تنها provider اجتماعی برای اتصال رسمی حساب، انتشار، زمان‌بندی،
Inbox، comment-to-DM، Insight و دریافت صدای کاتالوگ‌شده است. پادو با Playbook و inference
مستقل DeepInfra تحلیل، راهبرد و تولید را انجام می‌دهد؛ WooCommerce منبع حقیقت تجارت و
بک‌اند IGBZ مرجع مجوز، صف و audit است. scraper، cookie، session مرورگر و Graph API مستقیم
در محصول ممنوع‌اند.

**وضعیت کد:** سرویس‌ها و endpointهای فعلی هنوز برای Manus و ManyChat/ChatPlace نوشته شده‌اند. پیشنهاد
کانال Instagram مبتنی بر session در Agent Reach نیز رد شده و نباید وارد کد شود. جدول‌های زیر موجودی کد legacy را ثبت می‌کنند و به معنای پذیرش آن‌ها
نیستند. migration به Zernio و adapterهای پادو هنوز پیاده‌سازی نشده است.

### ۳.۱ کد legacy تولید و انتشار محتوا — نیازمند migration
...
این مسیر legacy از `PromptBuilder`، webhook/polling و اعتبار داخلی Manus استفاده می‌کند.
در معماری هدف، چهار اجرای Playbook از `PROMPT-IG-GROWTH-PADO.md` و adapter مستقل
DeepInfra جای آن را می‌گیرد و انتشار فقط از صف Zernio انجام می‌شود؛ این migration هنوز کد
ندارد.

### ۳.۲ کد legacy قیف کامنت→دایرکت — نیازمند migration
...
این سوییچ و هر دو adapter مربوط به کد legacy هستند و باید حذف/مهاجرت داده شوند. در هدف،
Zernio رویداد Comment/DM را با webhook امضاشده وارد می‌کند؛ بک‌اند IGBZ rule، محدودیت
per-user، idempotency، پاداش `instagram_reward` و audit را اعمال می‌کند و سپس پاسخ را از
Zernio می‌فرستد. `PostIdentity` می‌تواند پس از تطبیق قرارداد Zernio حفظ شود؛ `''` همچنان
نباید wildcard باشد.
...
برای ماژول چندمستأجری، ایجاد مستأجر، عضویت مالک، نگاشت دامنه، داشبورد مالک، محدوده‌بندی محصولات/سفارش‌ها و مدیریت محصول از رابط برنامه‌نویسی فعال شده است. برای ماژول پادو، درگاه بیرونی، صف مجوز، اعتبارسنجی ZIP، پیش‌نمایش، اعمال و بازگشت قالب فعال شده‌اند. مواردی که در جدول‌های قدیمی با عنوان «موجود» آمده‌اند همچنان باید با کلید واقعی سرویس بیرونی و آزمون زنده تأیید شوند.
