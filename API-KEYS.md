...
## ۴.۵ credential پادوی بیرونی — تصمیم فاز ۰۲

طبق `ADR/ADR-0002-EXTERNAL-PADO-PER-STORE.md` هر زیرسایت حساب/اشتراک پادوی بیرونی مستقل
خواهد داشت. `pado.api_key` در کد فعلی عضو رجیستری راز است، اما ذخیرهٔ کنونی هنوز قرارداد
نهایی per-site نیست و نباید به‌عنوان کلید مشترک شبکه استفاده شود. در معماری مقصد:

- credential فقط به همان زیرسایت و provider پادو تعلق دارد و جداگانه قابل ابطال است؛
- n8n و ویرا آن را برای فراخوانی مدل یا فروشگاه دیگر مصرف نمی‌کنند؛
- مقدار هیچ‌گاه در HTML، log، callback یا prompt بازنمایش نمی‌شود؛
- اینکه این مقدار کلید integration پادو است یا مدلِ متصل به حساب بیرونی، پس از انتخاب
  provider و قرارداد رسمی آن مشخص می‌شود.
