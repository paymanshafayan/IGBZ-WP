...
| NaraRouter / byNara Router | فعلاً فقط تست provider، نه تولید | سکرت GitHub Actions با نام `NARAROUTER_API_KEY`؛ در صورت ثبت دستی تولیدی: `pado.ai.key_vault` | OpenAI-compatible روی `https://router.bynara.id/v1`؛ smoke test بعدی پس از رفع گیت `telegram_required` با `glm-5.3-flash-free` است؛ تا قبل از benchmark فارسی، ZDR/حریم خصوصی و گیت‌های `enabled`/`benchmark_passed`/`geo_eligible` نباید مسیر تولیدی بگیرد |
