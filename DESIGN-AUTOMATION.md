...
وضعیت: سند طراحی مبنای پیاده‌سازی — پس از تطبیق با تصمیم‌های متأخر پروژه

⚠️ تصمیم‌های نهایی (۱۴۰۶/۰۵/۳۱):
- **انتخاب مدل و سرویس پادو با ماست** (نه با ادمین فروشگاه). ادمین فقط کلیدهای API مربوط به سرویس/مدل منتخب ما را تهیه و در پنل مدیریت ثبت می‌کند. انتخاب مدل بر اساس بهترین عملکرد در اتوماسیون فروشگاه (دقت ابزارفراخوانی، کیفیت خروجی فارسی/RTL، پایداری، هزینه، پشتیبانی از اسکیل/SKILL.md و ابزارها) صورت می‌گیرد.
- **المنتور و Hello Elementor از نصب پیش‌فرض حذف شدند** (حجم ~۱۰۸ مگابایت). هر فروشگاه با قالب پیش‌فرض هسته وردپرس بالا می‌آید؛ خروجی طراحی قالب پادو یک قالب فرزند FSE است.
...
- ChatPlace API + Manus (مسیر رسمی تعامل با اینستاگرام — بدون تماس مستقیم با Graph API)
- Meta Instagram API (فقط به‌عنوان مرجع ساختار داده؛ تماس مستقیم تا اطلاع ثانوی طبق قانون بنیادین پروژه ممنوع است)
...
- **پادو = سرویس ابری بیرونی** (مانند Gemini/ChatGPT) که ادمین هر فروشگاه خودش تهیه می‌کند. ما روی سرور خود **پروسه‌ای به‌ازای هر فروشگاه** برای پادو اجرا نمی‌کنیم؛ آنچه می‌سازیم: **(۱) هسته ویرا (Node.js) برای اتوماسیون داخلی + مدیریت اسکیل‌ها + نظارت + توزیع نسخه اسکیل‌ها**، **(۲) اسکیل (Skill) برای پادو** طبق فرمت `SKILL.md`، **(۳) هسته n8n برای orchestration وردفلوهای قطعی** (غیر LLM).
...
- **انتخاب مدل/سرویس پادو با ماست** (نه ادمین). ادمین فقط کلیدهای API مدل منتخب ما را در پنل ثبت می‌کند. انتخاب مدل بر اساس بهترین عملکرد در اتوماسیون (ابزارفراخوانی، کیفیت خروجی فارسی، پایداری، هزینه، پشتیبانی از Structured Output) انجام می‌شود و از طریق **API Gateway واحد در ویرا** در اختیار همهٔ وردفلوها/افزونه قرار می‌گیرد (هیچ قسمتی از سیستم مستقیماً با API مدل صحبت نمی‌کند).
...
نقش ویرا در این معماری:
- **دروازهٔ یکپارچه مدل** برای همهٔ Agentها و workflowهای n8n.
- **مدیریت اسکیل‌های پادو** (نسخه‌گذاری، توزیع، به‌روزرسانی، گزارش ماهیانه).
- **نظارت** (طبق تصمیم ۱۴۰۵/۰۵/۳۱ ویرا روی فروشگاه‌های زنده نظارت می‌کند ولی در
  کار روزمره دخالت نمی‌کند مگر در موارد انحراف شدید/فعالیت‌های مشکوک).
- **هیچ دسترسی مستقیم از بیرون به ویرا نیست**؛ فقط از طریق پنل/پراکسی داخلی با
  نشست مدیر احراز هویت‌شده قابل دسترسی است.

4.5 پادو (Hooshvareh)

پادو **لایه‌ای است که مدل/مدل‌های انتخابی ما** (بر اساس بهترین عملکرد در
اتوماسیون فروشگاه) را از طریق یک API یکپارچه در اختیار اسکیل‌ها و وردفلوها قرار
می‌دهد. **انتخاب مدل/سرویس با ماست**، نه با ادمین فروشگاه. ادمین فقط **کلید(های) API**
مربوط به سرویس منتخب را (که ما در اسناد راهنما اعلام می‌کنیم) تهیه و در پنل مدیریت
فروشگاه ثبت می‌کند. ما پادو را به‌عنوان یک **سرویس پلتفرمی میزبانی/مدیریت می‌کنیم**،
اما بار پردازش مدل ممکن است روی سرویس بیرونی (مثلاً Claude/GPT/Gemini) یا روی یک
مدل خودمیزبان اجرا شود.
...
- **درگاه یکپارچه (API Gateway)** در ویرا/سرور ما که همهٔ درخواست‌های n8n/افزونه
  را با احترام به بودجه/محدودیت نرخ/لاگ حسابرسی به مدل(ها) می‌فرستد.
...
⚠️ قانون بنیادین پروژه (۱۴۰۵/۰۵/۲۴): تماس مستقیم با Meta Graph API / Send API **تا
اطلاع ثانوی ممنوع است** (به‌دلیل محدودیت‌های دسترسی از ایران). تعامل با اینستاگرام از
طریق **Manus** (تولید/انتشار/تحلیل محتوا) و **ChatPlace** (قیف کامنت→دایرکت +
MCP در آینده) انجام می‌شود. Windsor (عامل مرورگر) بررسی و رد شد.

اجزا:

1. Content Planner
2. Content Creator (با استفاده از ویرا/مدل)
3. Publishing (از طریق Manus)
4. Engagement (دریافت کامنت/دایرکت از ChatPlace، پاسخ پیشنهادی → تأیید/ارسال)
5. Analytics (از ChatPlace/Manus)
...
Caption / Hook / CTA
   |
Brand Check (با اسکیل brand مرجع ui-ux-pro-max)
   |
Product Fact Check (از Source of Truth ووکامرس)
   |
Manager Approval (الزامی — این یک عملیات HIGH است)
   |
Instagram Publish (از طریق Manus؛ نه تماس مستقیم Graph API)
   |
Record Media ID (در جدول `ig_content` پلاگین IGBZ)
   |
Analytics (از ChatPlace / Manus به‌صورت تجمیعی)

محدودیت: در MVP (بند ۳۴) اینستاگرام **بعد از پایداری هسته اصلی** اضافه می‌شود.
...
Read:
Content / Analytics (فقط از طریق Manus/ChatPlace API؛ تماس مستقیم Graph API ممنوع)

Write:
Draft (پیش‌نویس کپشن/تصویر/برنامه انتشار)

Publish:
فقط با Approval و فقط از طریق Manus (نه مستقیم)

همهٔ عامل‌ها:
- DELETE به‌صورت پیش‌فرض ممنوع مگر با مجوز صریح و Soft Delete اولیه.
- هرگونه تماس با شبکه خارج از Workflow ثبت می‌شود.
- اعتبار کلیدهای API فقط در ویرا نگهداری می‌شوند؛ n8n از SDK ویرا استفاده می‌کند.
- دسترسی به MCP سرورها فقط برای MCPهای تأییدشده در لیست سفید.
...
- Credentials (درگاه‌های ایرانی، ChatPlace، Manus، مدل‌ها) — همه در ویرا، نه در n8n
...
42.1 لایه‌ها (قاعده — به‌روز شده ۱۴۰۶/۰۵/۳۱)
- هستهٔ اتوماسیون (n8n + ویرا + درگاه یکپارچه پادو) روی سرور ما و **فقط برای
  مدیران ما / سیستم ما** است.
- **انتخاب مدل/سرویس پادو با ماست**. ادمین فروشگاه فقط کلیدهای API مربوطه
  (که ما مشخص می‌کنیم) را تهیه و در پنل ثبت می‌کند. ما بر اساس بهترین عملکرد
  در اتوماسیون (ابزارفراخوانی، کد، فارسی، هزینه، پایداری) مدل(ها) را انتخاب و
  در صورت نیاز تغییر می‌دهیم.
- ما اسکیل‌های پادو را می‌نویسیم، نسخه‌گذاری و به‌طور خودکار توزیع می‌کنیم.
  کلید API ادمین از طریق دروازه ما استفاده می‌شود (احتمال سوءاستفاده صفر).
...
==================================================
44. مرز قطعی پادو، n8n، مدل و ویرا — ۱۴۰۵/۰۶/۰۵
==================================================

مرجع جاری `ADR/ADR-0002-EXTERNAL-PADO-PER-STORE.md` است:
...
به‌تنهایی provider، خروج داده، پروتکل اتصال یا نوع خروجی قالب را تعیین نمی‌کند.
